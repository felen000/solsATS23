...
while cv2.waitKey(1) != 27:
    ...
    # цикл перебора найденных групп полос и определения соответствующих им условий
    #
    for i in range(len(groups)):
        # Получаем текущую группу полос маркировки кубика
        group = groups[i]
        # Флаг, указывающий, стоит ли продолжать искать группу маркировки или она уже найдена
        flg = False
        # Определяем, относится ли кубик к группе с двумя цветами
        for i2 in group:
            if len(group[i2]) == 2:  # если да
                # выделяем его на изображении
                cv2.circle(img, (group[i2][0][0], group[i2][0][1]), radius=5, color=(0, 255, 255), thickness=-1)
                flg = True
                break
        if flg:
            continue
            
        # Определяем, что кубик относится к группе без зеленых полос
        for i2 in group:
            if i2 == "green":
                break
        else: # если да
            for i2 in group:
                # выделяем его на изображении
                cv2.circle(img, (group[i2][0][0], group[i2][0][1]), radius=5, color=(255, 0, 255), thickness=-1)
                flg = True
                break
        if flg:
            continue

        # Определяем, что кубик относится к группе, где маркировка начинается с белого цвета
        for i2 in group:
            ms = []
            for i in group[i2]:
                ms.append(i[0])
            if ms[0] != ms[1]:
                ...
                flg_is_nachalo = False
            if i2 == "white" and len(group[i2]) == 1 and flg_is_nachalo: # если да
                # выделяем его на изображении
                cv2.circle(img, (group[i2][0][0], group[i2][0][1]), radius=5, color=(0, 0, 0), thickness=-1)
                flg = True
                break
        if flg:
            continue

        # если flg до сих пор False, значит мы определили, что кубик не относится ни к какой из 3-х групп
        for i2 in group:
            # выделяем кубик, который не относится ни к какой из 3-х групп группе, на изображении
            cv2.circle(img, (group[i2][0][0], group[i2][0][1]), radius=5, color=(255, 255, 0), thickness=-1)
            break
    cv2.imshow("IMG", img)
